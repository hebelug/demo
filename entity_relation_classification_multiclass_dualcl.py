...
class RelationDataset(Dataset):
    def __init__(self, json_data, tokenizer, max_length=128):
        ...
        self.label_names = ["No relation", "Effect", "Mechanism", "Advise"]  # 标签名称
        self.label_map = {"NA": 0, "effect": 1, "mechanism": 2, "advise": 3}  # 标签映射
...
class PromptDualCLBERT(nn.Module):
    def __init__(self, bert_model, num_labels=4, temperature=0.07):
        super(PromptDualCLBERT, self).__init__()
        self.bert = bert_model
        self.num_labels = num_labels
        self.temperature = temperature
        self.dropout = nn.Dropout(0.1)
        self.label_tokens = ["No relation", "Effect", "Mechanism", "Advise"]  # 标签名称
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        # 标签 token 位置：No relation (1-2), Effect (3), Mechanism (4), Advise (5)
        theta = []
...
def test_model(model, test_loader, device):
    ...
    model.eval()
    predictions = []
    label_names = ["No relation", "Effect", "Mechanism", "Advise"]
    with torch.no_grad():
...
def main():
    ...
    model = PromptDualCLBERT(bert_model, num_labels=4).to(device)
