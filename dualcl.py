...
class DualCLBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        # 分类 logits
        logits = self.classifier(z)  # [batch_size, num_labels]
        ...
        if labels is not None:
            ...
            for i in range(z.shape[0]):
                ...
                pos_scores = torch.exp(anchor_z @ pos_z.t() / self.temperature)
                neg_scores = torch.exp(anchor_z @ neg_z.t() / self.temperature)
                l_z += -torch.log(pos_scores.sum() / (pos_scores.sum() + neg_scores.sum() + 1e-8)).mean()

            # 计算 \mathcal{L}_\theta: 使用相同的 [CLS] 嵌入
            l_theta = 0
            for i in range(z.shape[0]):
                anchor_z = z_norm[i]
                pos_indices = (labels == labels[i]).nonzero(as_tuple=True)[0]
...
def train_model(model, train_loader, valid_loader, test_loader, optimizer, device, epochs=3, lambda_dual=1.0):
    ...
    for epoch in range(epochs):
        ...
        # 验证集评估
        valid_loss, valid_accuracy, valid_precision, valid_recall, valid_f1 = evaluate_model(model, valid_loader,
                                                                                             device)
...
def main():
    ...
    # 加载 BioBERT v1.1 模型和分词器（从 Hugging Face 下载）
    ...
    model = DualCLBERT(bert_model, num_labels=5).to(device)
