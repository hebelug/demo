...
def train_model(model, train_loader, valid_loader, optimizer, device, epochs=3, lambda_dual=1.0):
    ...
    for epoch in range(epochs):
        ...
        if valid_f1 > best_f1:
            best_f1 = valid_f1
            best_model_path = os.path.join('save_model', f'best_model_f1_{best_f1:.4f}.pt')
            torch.save(model.state_dict(), best_model_path)
            print(f"Saved best model with Validation F1: {best_f1:.4f} at epoch {epoch+1}")
...
# 测试函数
def test_model(model, test_loader, device, best_model_path, best_f1):
    # 加载最佳模型
    ...
    # 将预测结果写入日志文件
    log_path = os.path.join('save_model', f'predictions_f1_{best_f1:.4f}.log')
...
def main():
    ...
    # 训练模型并获取最佳模型路径和 F1 分数
    best_model_path, best_f1 = train_model(model, train_loader, valid_loader, optimizer, device)

    # 测试模型
    print("\nTesting on test set with best model:")
    test_model(model, test_loader, device, best_model_path, best_f1)
