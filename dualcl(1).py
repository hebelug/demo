...
def train_model(model, train_loader, val_loader, optimizer, epochs: int = 3, model_save_path: str = "models"):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_pbar:
            ...
            total_z_loss += loss_z.item()
            total_theta_loss += loss_theta.item()
        ...
        avg_theta_loss = total_theta_loss / len(train_loader)
        logger.info(f"Epoch {epoch+1}/{epochs} - Total Loss: {avg_loss:.4f}, Z Loss: {avg_z_loss:.4f}, Theta Loss: {avg_theta_loss:.4f}")
