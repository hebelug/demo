...
class RelationDataset(Dataset):
    def __init__(self, json_data, tokenizer, max_length=128):
        ...
        self.label_names = ["No relation", "Relation exists"]  # 标签名称
...
class PromptDualCLBERT(nn.Module):
    def __init__(self, bert_model, num_labels=2, temperature=0.07):
        super(PromptDualCLBERT, self).__init__()
        self.bert = bert_model
        self.num_labels = num_labels
        self.temperature = temperature
        self.dropout = nn.Dropout(0.1)
        self.label_tokens = ["No relation", "Relation exists"]  # 标签名称
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        # 假设标签 token 在输入序列的开头（位置 1 和 2，因为 0 是 [CLS]）
        theta = []
        for i in range(self.num_labels):
            # 对于多词标签，取平均池化
            label_tokens = self.label_tokens[i].split()
            label_indices = list(range(1, 1 + len(label_tokens)))  # 标签 token 位置
        ...
        if labels is not None:
            ...
            for i in range(z.shape[0]):  # 遍历每个样本
                ...
                neg_theta = theta_norm[neg_indices, labels[neg_indices]]  # 负样本的 \theta^*
...
def main():
    ...
    model = PromptDualCLBERT(bert_model).to(device)
