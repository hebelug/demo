...
def test_model(model, test_loader, device, best_model_path):
    ...
    # 将预测结果写入日志文件
    with open('predictions.log', 'w') as f:
...
def main():
    ...
    # 训练模型并获取最佳模型路径
    best_model_path = train_model(model, train_loader, valid_loader, optimizer, device)

    # 测试模型
    print("\nTesting on test set with best model:")
    test_model(model, test_loader, device, best_model_path)
